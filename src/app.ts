...
export const app = b.createVirtualComponent({
    ...
    render(appCtx: AppCtx, me: b.IBobrilNode) {
        me.children = bs.container({ style: bs.typography.textCenter }, [
            ...
            bs.textarea({ value: log.get().join('\n'), rows: 20, fixedSize: true, readonly: true }),
            bs.row({}, bs.button({
            ...
            bs.row({}, bs.button({
                disabled: !api.isAvailable() || appCtx.showAllPaths || appCtx.runner.foundPath === undefined,
                label: 'SHOW PATHS',
...
function getMatrix(appCtx: AppCtx) {
    ...
    let content = [path({ points: appCtx.runner.foundPath.verticies, isHighlited: true })];
    if (appCtx.showAllPaths)
        content = [...appCtx.matrixCache, content];
    else
        content.unshift(path({
            points: [{ x: 0, y: 0 }, { x: 0, y: 300 }, { x: 300, y: 300 }, { x: 300, y: 0 }, { x: 0, y: 0 }],
            isHighlited: false
        }));
